...
local function ValidateKey(key, playerName)
    ...
    return true, "Key verificada correctamente", key, rem
end

-- ====================================================================
-- EJECUCIÓN DEL PANEL TRAS INICIAR SESIÓN CON ÉXITO
-- ====================================================================
local function OnSuccess(key, remainingSeconds)

----------------------------------------------------------------------
------- aqui Pon El Codigo del panel -------
----------------------------------------------------------------------

    print("¡Acceso Concedido! Cargando el panel...")

    -- OPCIÓN A: Si cargas tu panel desde un link HTTP (GitHub / Raw script):
    -- loadstring(game:HttpGet("https://raw.githubusercontent.com/TU_USUARIO/TU_REPO/main/script.lua"))()

    -- OPCIÓN B: Si tienes todo el código de tu panel aquí, pégalo directamente abajo:
...
-- Clic en Validar (Con Mensaje + Sonido + Animación de cierre)
validateBtn.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if valid then
            -- 1. Cambiar texto y color a Verde Exitoso
            validateBtn.Text = "Key verificada correctamente"
            validateBtn.BackgroundColor3 = Color3.fromRGB(34, 197, 94)

            -- 2. Reproducir Efecto de Sonido
            pcall(function()
                ...
                task.delay(3, function()
                    if successSound then successSound:Destroy() end
                end)
            end)

            -- 3. Breve pausa para ver la confirmación
            task.wait(0.6)
            ...
            -- 5. Destruir Login GUI y Ejecutar el Panel
            mainGui:Destroy()
